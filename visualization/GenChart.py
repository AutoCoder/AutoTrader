#!/usr/bin/env python
import sys
import MySQLdb

#CONFIG
dbhost = '127.0.0.1'
dbuser = 'root'
dbpassword = 'wodemima'
port = 3306

def getK3K5Data(date, instrumentID):
	"""
		[new Date(1970, 10, 5, 12, 30, 15, 100), 10] ,
		for item in results:
			print item
	"""
	print date,instrumentID
	try:
		conn=MySQLdb.connect(host=dbhost,user=dbuser,passwd=dbpassword,port=port)
		cur=conn.cursor()
		
		conn.select_db('qihuo')

		sql = "select `k3m`,`k5m`,`UpdateTime`,`UpdateMillisec` from %s where `Date` = '%s' limit 10" % (instrumentID, date)

		print sql

		count=cur.execute(sql)
		print 'there has %s rows record' % count

		results=cur.fetchall()

		print results


	except MySQLdb.Error,e:
		print "Mysql Error %d: %s" % (e.args[0], e.args[1])
		return None

def getVolumeData(date, instrumentID):
	print date,instrumentID
	pass


def plotAll():
	#plt.figure(1)
	klinesList = []
	
	for i in xrange(10):
		klinesList.append( (i, random.uniform(14, 15), random.uniform(14, 15), random.uniform(2000,3000)) ) 	
	print klinesList

	tick = [x[0] for x in klinesList]
	#print(years)
	k3price = [x[1] for x in klinesList]
	#print(price)
	k5price = [x[2] for x in klinesList]	
	volume = [x[3] for x in klinesList]
	print volume
	ax1 = plt.subplot(211)
	ax2 = plt.subplot(212)

	plt.sca(ax1)
	plt.plot(tick, k3price, 'b*')#,label="$cos(x^2)$")
	plt.plot(tick, k3price, 'r')

	plt.plot(tick, k5price, 'y*')#,label="$cos(x^2)$")
	plt.plot(tick, k5price, 'g')

	plt.xlabel("Tick")
	plt.ylabel("K3-k5")
	plt.ylim(10, 20)
	plt.title('line_regression & gradient decrease')
	plt.legend()

	plt.sca(ax2)
	for item in klinesList:
		plt.bar(item[0], item[3], color='r', width=0.2)

	# plt.xticks(tick)
	# plt.yticks(volume)
	plt.grid(True)
	# data = np.random.randn(1000)
	# n, bins = np.histogram(data, 50)

	# left = np.array(bins[:-1])
	# right = np.array(bins[1:])
	# bottom = np.zeros(len(left))
	# top = bottom + n

	# plt.xlabel("Tick(+2000)")
	# plt.ylabel("Volume")
	# print volume
	# print tick

	data = [1,2,3,1,1,1,2,4,3,1,]
	print tick
	#for i in xrange(0, 10):
	#	plt.bar(tick[i], volume[i], color='r', width=0.8)
	# print data
	# print tick
	#plt.bar(data,tick, color='r',width=0.2)
	#plt.xticks()
	#ax2.hist(data, bins=tick)#, rwidth=0.8normed=1, histtype='bar', 
	#plt.hist(years, volume)

	plt.show()

def k3k5Data():
	return """{
					title : {
						text: 'K-3min & K-5min',
					},
					tooltip : {
						trigger: 'axis'
					},
					legend: {
						data:['K-5min','K-3min']
					},
					toolbox: {
						show : true,
						feature : {
							mark : {show: true},
							dataView : {show: true, readOnly: false},
							restore : {show: true},
							saveAsImage : {show: true}
						}
					},
					dataZoom : {
						show : true,
						realtime: true,
						start : 0,
						end : 100
					},
					xAxis : [
						{
							type : 'time',
							splitNumber:5
						}
					],
					yAxis : [
						{
							type : 'value',
							scale : true,
							axisLabel : {
								formatter: '{value}k RMB'
							}
						}
					],
					series : [
						{
							name:'K-5min',
							type:'line',
							data: [ [new Date(1970, 10, 5, 12, 30, 15, 100), 10] , [new Date(1970, 10, 5, 13, 00, 15, 100), 3], [new Date(1970, 10, 5, 13, 30, 15, 100), 7], [new Date(1970, 10, 5, 14, 00, 15, 100), 3] , [new Date(1970, 10, 5, 14, 30, 15, 100), 5], [new Date(1970, 10, 5, 15, 00, 15, 100), 9],  [new Date(1970, 10, 5, 16, 30, 15, 100), 3] , [new Date(1970, 10, 5, 16, 50, 15, 100), 5], [new Date(1970, 10, 5, 17, 00, 15, 100), 9] ],
							markPoint : {
								symbol: 'heart',
								//symbolSize:20,
								itemStyle:{
									normal:{label:{position:'top'}}
								},
								data : [
									{name : 'SellPoint', value : 5, xAxis: new Date(1970, 10, 5, 14, 30, 15, 100), yAxis: 5}
								]
							}
						},
						{
							name:'K-3min',
							type:'line',
							data: [ [new Date(1970, 10, 5, 12, 30, 15, 100), 3] , [new Date(1970, 10, 5, 13, 00, 15, 100), 5], [new Date(1970, 10, 5, 13, 30, 15, 100), 9], [new Date(1970, 10, 5, 14, 00, 15, 100), 10] , [new Date(1970, 10, 5, 14, 30, 15, 100), 3], [new Date(1970, 10, 5, 15, 00, 15, 100), 7], [new Date(1970, 10, 5, 16, 30, 15, 100), 10] , [new Date(1970, 10, 5, 16, 50, 15, 100), 3], [new Date(1970, 10, 5, 17, 00, 15, 100), 7] ],
							markPoint : {
								symbol: 'star',
								//symbolSize:20,
								itemStyle:{
									normal:{label:{position:'top'}}
								},
								data : [
									{name : 'BuyPoint', value : 3, xAxis: new Date(1970, 10, 5, 13, 00, 15, 100), yAxis: 3}
								]
							}
						}
					]
				};"""


def renderktempl():
	return """{
					title : {
						text: 'K-Chart',
					},
					tooltip : {
						trigger: 'axis',
						formatter: function (params) {
							var res = params[0].seriesName + ' ' + params[0].name;
							res += '<br/>  Open : ' + params[0].value[0] + '  Highest : ' + params[0].value[3];
							res += '<br/>  Close : ' + params[0].value[1] + '  Lowest : ' + params[0].value[2];
							return res;
						}
					},
					toolbox: {
						show : true,
						feature : {
							mark : {show: true},
							dataZoom : {show: true},
							dataView : {show: true, readOnly: false},
							restore : {show: true},
							saveAsImage : {show: true}
						}
					},
					dataZoom : {
						show : true,
						realtime: true,
						start : 0,
						end : 50
					},
					xAxis : [
						{
							type : 'category',
							boundaryGap : true,
							axisTick: {onGap:false},
							splitLine: {show:false},
							data : [
								"2013/1/24", "2013/1/25", "2013/1/28", "2013/1/29", "2013/1/30",
								"2013/1/31", "2013/2/1", "2013/2/4", "2013/2/5", "2013/2/6", 
								"2013/2/7", "2013/2/8", "2013/2/18", "2013/2/19", "2013/2/20", 
								"2013/2/21", "2013/2/22", "2013/2/25", "2013/2/26", "2013/2/27", 
								"2013/2/28", "2013/3/1", "2013/3/4", "2013/3/5", "2013/3/6", 
								"2013/3/7", "2013/3/8", "2013/3/11", "2013/3/12", "2013/3/13", 
								"2013/3/14", "2013/3/15", "2013/3/18", "2013/3/19", "2013/3/20", 
								"2013/3/21", "2013/3/22", "2013/3/25", "2013/3/26", "2013/3/27", 
								"2013/3/28", "2013/3/29", "2013/4/1", "2013/4/2", "2013/4/3", 
								"2013/4/8", "2013/4/9", "2013/4/10", "2013/4/11", "2013/4/12", 
								"2013/4/15", "2013/4/16", "2013/4/17", "2013/4/18", "2013/4/19", 
								"2013/4/22", "2013/4/23", "2013/4/24", "2013/4/25", "2013/4/26", 
								"2013/5/2", "2013/5/3", "2013/5/6", "2013/5/7", "2013/5/8", 
								"2013/5/9", "2013/5/10", "2013/5/13", "2013/5/14", "2013/5/15", 
								"2013/5/16", "2013/5/17", "2013/5/20", "2013/5/21", "2013/5/22", 
								"2013/5/23", "2013/5/24", "2013/5/27", "2013/5/28", "2013/5/29", 
								"2013/5/30", "2013/5/31", "2013/6/3", "2013/6/4", "2013/6/5", 
								"2013/6/6", "2013/6/7", "2013/6/13"
							]
						}
					],
					yAxis : [
						{
							type : 'value',
							scale:true,
							boundaryGap: [0.01, 0.01]
						}
					],
					series : [
						{
							name:'SH stock value',
							type:'k',
							barMaxWidth: 20,
							itemStyle: {
								normal: {
									color: 'red',		   
									color0: 'lightgreen',   
									lineStyle: {
										width: 2,
										color: 'orange',	
										color0: 'green'	 
									}
								},
								emphasis: {
									color: 'black',		 
									color0: 'white'		 
								}
							},
							data:[ 
								{
									value:[2320.26,2302.6,2287.3,2362.94],
									itemStyle: {
										normal: {
											color0: 'blue',		 
											lineStyle: {
												width: 3,
												color0: 'blue'	  
											}
										},
										emphasis: {
											color0: 'blue'		  
										}
									}
								},
								[2300,2291.3,2288.26,2308.38],
								[2295.35,2346.5,2295.35,2346.92],
								[2347.22,2358.98,2337.35,2363.8],
								[2360.75,2382.48,2347.89,2383.76],
								[2383.43,2385.42,2371.23,2391.82],
								[2377.41,2419.02,2369.57,2421.15],
								[2425.92,2428.15,2417.58,2440.38],
								[2411,2433.13,2403.3,2437.42],
								[2432.68,2434.48,2427.7,2441.73],
								[2430.69,2418.53,2394.22,2433.89],
								[2416.62,2432.4,2414.4,2443.03],
								[2441.91,2421.56,2415.43,2444.8],
								[2420.26,2382.91,2373.53,2427.07],
								[2383.49,2397.18,2370.61,2397.94],
								[2378.82,2325.95,2309.17,2378.82],
								[2322.94,2314.16,2308.76,2330.88],
								[2320.62,2325.82,2315.01,2338.78],
								[2313.74,2293.34,2289.89,2340.71],
								[2297.77,2313.22,2292.03,2324.63],
								[2322.32,2365.59,2308.92,2366.16],
								[2364.54,2359.51,2330.86,2369.65],
								[2332.08,2273.4,2259.25,2333.54],
								[2274.81,2326.31,2270.1,2328.14],
								[2333.61,2347.18,2321.6,2351.44],
								[2340.44,2324.29,2304.27,2352.02],
								[2326.42,2318.61,2314.59,2333.67],
								[2314.68,2310.59,2296.58,2320.96],
								[2309.16,2286.6,2264.83,2333.29],
								[2282.17,2263.97,2253.25,2286.33],
								[2255.77,2270.28,2253.31,2276.22],
								[2269.31,2278.4,2250,2312.08],
								[2267.29,2240.02,2239.21,2276.05],
								[2244.26,2257.43,2232.02,2261.31],
								[2257.74,2317.37,2257.42,2317.86],
								[2318.21,2324.24,2311.6,2330.81],
								[2321.4,2328.28,2314.97,2332],
								[2334.74,2326.72,2319.91,2344.89],
								[2318.58,2297.67,2281.12,2319.99],
								[2299.38,2301.26,2289,2323.48],
								[2273.55,2236.3,2232.91,2273.55],
								[2238.49,2236.62,2228.81,2246.87],
								[2229.46,2234.4,2227.31,2243.95],
								[2234.9,2227.74,2220.44,2253.42],
								[2232.69,2225.29,2217.25,2241.34],
								[2196.24,2211.59,2180.67,2212.59],
								[2215.47,2225.77,2215.47,2234.73],
								[2224.93,2226.13,2212.56,2233.04],
								[2236.98,2219.55,2217.26,2242.48],
								[2218.09,2206.78,2204.44,2226.26],
								[2199.91,2181.94,2177.39,2204.99],
								[2169.63,2194.85,2165.78,2196.43],
								[2195.03,2193.8,2178.47,2197.51],
								[2181.82,2197.6,2175.44,2206.03],
								[2201.12,2244.64,2200.58,2250.11],
								[2236.4,2242.17,2232.26,2245.12],
								[2242.62,2184.54,2182.81,2242.62],
								[2187.35,2218.32,2184.11,2226.12],
								[2213.19,2199.31,2191.85,2224.63],
								[2203.89,2177.91,2173.86,2210.58],
								[2170.78,2174.12,2161.14,2179.65],
								[2179.05,2205.5,2179.05,2222.81],
								[2212.5,2231.17,2212.5,2236.07],
								[2227.86,2235.57,2219.44,2240.26],
								[2242.39,2246.3,2235.42,2255.21],
								[2246.96,2232.97,2221.38,2247.86],
								[2228.82,2246.83,2225.81,2247.67],
								[2247.68,2241.92,2231.36,2250.85],
								[2238.9,2217.01,2205.87,2239.93],
								[2217.09,2224.8,2213.58,2225.19],
								[2221.34,2251.81,2210.77,2252.87],
								[2249.81,2282.87,2248.41,2288.09],
								[2286.33,2299.99,2281.9,2309.39],
								[2297.11,2305.11,2290.12,2305.3],
								[2303.75,2302.4,2292.43,2314.18],
								[2293.81,2275.67,2274.1,2304.95],
								[2281.45,2288.53,2270.25,2292.59],
								[2286.66,2293.08,2283.94,2301.7],
								[2293.4,2321.32,2281.47,2322.1],
								[2323.54,2324.02,2321.17,2334.33],
								[2316.25,2317.75,2310.49,2325.72],
								[2320.74,2300.59,2299.37,2325.53],
								[2300.21,2299.25,2294.11,2313.43],
								[2297.1,2272.42,2264.76,2297.1],
								[2270.71,2270.93,2260.87,2276.86],
								[2264.43,2242.11,2240.07,2266.69],
								[2242.26,2210.9,2205.07,2250.63],
								[2190.1,2148.35,2126.22,2190.1]
							],
							markPoint : {
								symbol: 'star',
								//symbolSize:20,
								itemStyle:{
									normal:{label:{position:'top'}}
								},
								data : [
									{name : 'Highest', value : 2444.8, xAxis: '2013/2/18', yAxis: 2466}
								]
							}
						}
					]
				};"""

def renderbartempl():
	return  """{
					title : {
						text: 'Exchange Volume',
					},
					dataZoom : {
						show : true,
						realtime: true,
						start : 0,
						end : 100
					},
					xAxis : [
						{
							type : 'time',
							splitNumber:5
						}
					],
					yAxis : [
						{
							type : 'value'
						}
					],
					series : [
						{
							"name":"Exchange Volume",
							"type":"bar",
							"data":[ [new Date(1970, 10, 5, 12, 30, 15, 100), 1000] , [new Date(1970, 10, 5, 13, 00, 15, 100), 800], [new Date(1970, 10, 5, 13, 30, 15, 100), 3000], [new Date(1970, 10, 5, 14, 00, 15, 100), 5000] , [new Date(1970, 10, 5, 14, 30, 15, 100), 1500], [new Date(1970, 10, 5, 15, 00, 15, 100), 1900], [new Date(1970, 10, 5, 16, 30, 15, 100), 2100] , [new Date(1970, 10, 5, 16, 50, 15, 100), 4000], [new Date(1970, 10, 5, 17, 00, 15, 100), 3500] ],
						}
					]
				};
			"""
def testMoko():
	from mako.template import Template
	mytemplate = Template(filename='./template/templ.html')
	f = open('kchart.html', 'w')
	f.write(mytemplate.render(option1=k3k5Data(), option2=renderktempl(), option3=renderbartempl()))
	f.close()	

#usage:  cmd instrumentId date	   --- cmd "rb1510" "20150510"
if __name__ == "__main__":
	print sys.argv
	#testMoko()
	if (len(sys.argv) == 3):
		data = getData(sys.argv[2], sys.argv[1])
	#plotAll()
	getK3K5Data(date='2015-05-13', instrumentID='rb1510')